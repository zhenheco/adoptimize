...
export function AIQuotaCard({ copywriting, image }: AIQuotaCardProps) {
  ...
  function getQuotaTextColor(isCritical: boolean, isWarning: boolean): string {
      ...
      <div className="space-y-2">
        ...
        {isUnlimited ? (
          ...
          <div className="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
            <div
              className={`h-full transition-all rounded-full ${getQuotaBarColor(isCritical, isWarning)}`}
        ...
        {isUnlimited ? (
          ...
          <p className={`text-xs ${getQuotaTextColor(isCritical, isWarning)}`}>
            {getQuotaMessage(isCritical, isWarning, quota.remaining)}
          </p>
        )}
      </div>
    );
