...
export async function GET(request: NextRequest) {
  try {
    ...
    const redirectUri = `${request.nextUrl.origin}/api/v1/auth/oauth/meta/callback`;

    const response = await fetch(
      `${PYTHON_API_URL}/api/v1/auth/oauth/meta?redirect_uri=${encodeURIComponent(redirectUri)}`,
      {
        method: 'GET',
        headers: {
