...
export async function GET(request: NextRequest) {
  try {
    ...
    params.set('redirect_uri', `${request.nextUrl.origin}/api/v1/accounts/callback/google`);

    const response = await fetch(
      `${PYTHON_API_URL}/api/v1/accounts/connect/google/callback?${params.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    // 成功後重定向到帳戶頁面
    return NextResponse.redirect(
      new URL(`/accounts?success=google&account_id=${data.account_id}`, request.url)
    );
  } catch (error) {
