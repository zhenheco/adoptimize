/**
 * Meta Marketing API OAuth 回調端點
 *
 * 處理 Meta OAuth 授權後的回調，
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 處理 OAuth 錯誤 (Meta 使用 error_reason)
    if (error) {
    ...
    params.set('redirect_uri', `${request.nextUrl.origin}/api/v1/accounts/callback/meta`);

    const response = await fetch(
      `${PYTHON_API_URL}/api/v1/accounts/connect/meta/callback?${params.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    // 成功後重定向到帳戶頁面
    return NextResponse.redirect(
      new URL(`/accounts?success=meta&account_id=${data.account_id}`, request.url)
    );
  } catch (error) {
