/**
 * LinkedIn Ads OAuth 回調端點
 *
 * 處理 LinkedIn OAuth 授權後的回調，
...
export async function GET(request: NextRequest) {
  try {
    ...
    params.set('redirect_uri', `${request.nextUrl.origin}/api/v1/accounts/callback/linkedin`);

    const response = await fetch(
      `${PYTHON_API_URL}/api/v1/accounts/connect/linkedin/callback?${params.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok || !data.success) {
      const errorMessage = data.error || data.detail || 'LinkedIn OAuth callback failed';
      return NextResponse.redirect(
    ...
    // 成功後重定向到帳戶頁面
    return NextResponse.redirect(
      new URL(`/accounts?success=linkedin&account_id=${data.account_id}`, request.url)
    );
  } catch (error) {
