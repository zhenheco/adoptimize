/**
 * Pinterest Ads OAuth 回調端點
 *
 * 處理 Pinterest OAuth 授權後的回調，
...
export async function GET(request: NextRequest) {
  try {
    ...
    params.set('redirect_uri', `${request.nextUrl.origin}/api/v1/accounts/callback/pinterest`);

    const response = await fetch(
      `${PYTHON_API_URL}/api/v1/accounts/connect/pinterest/callback?${params.toString()}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok || !data.success) {
      const errorMessage = data.error || data.detail || 'Pinterest OAuth callback failed';
      return NextResponse.redirect(
    ...
    // 成功後重定向到帳戶頁面
    return NextResponse.redirect(
      new URL(`/accounts?success=pinterest&account_id=${data.account_id}`, request.url)
    );
  } catch (error) {
